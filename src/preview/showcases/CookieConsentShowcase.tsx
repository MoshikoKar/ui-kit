...
export const CookieConsentShowcase: React.FC = () => {
  ...
  useEffect(() => {
    const consent = localStorage.getItem('cookie-consent-preview');
    if (!consent) {
  ...
  const handleReset = () => {
    localStorage.removeItem('cookie-consent-preview');
    setShowConsent(true);
    setShouldShowModal(true);
  };
  ...
  const handleOptions = () => {
    ...
    alert('More options clicked - this would typically open a detailed preferences modal');
  };
  ...
  return (
    <>
      {shouldShowModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 dark:bg-black/40 transition-opacity duration-300"
        >
          <CookieConsent
            ...
            storageKey="cookie-consent-preview"
      ...
      <ShowcaseSection
        ...
        description="A configurable cookie consent component with localStorage persistence"
        id="cookie-consent"
      >
        <div className="space-y-4">
          <div className="p-4 rounded-lg border border-border bg-surface-secondary">
            <p className="text-sm text-text-secondary mb-4">
              The cookie consent component automatically checks localStorage and only displays if consent hasn't been given.
              Click the button below to reset the consent state and see the component again.
            </p>
            <Button
              variant="secondary"
              size="sm"
              onClick={handleReset}
            >
              Reset Consent & Show Component
            </Button>
          </div>

          <div className="p-4 rounded-lg border border-border bg-surface-secondary">
            <h3 className="text-sm font-semibold text-text-primary mb-2">Inline Usage Example</h3>
            <div className="flex justify-center items-center min-h-[200px]">
              <CookieConsent
                ...
                storageKey="cookie-consent-inline-preview"
                storageValue="accepted"
                icon={defaultIcon}
                privacyLinkTarget="_blank"
                privacyLinkRel="noopener noreferrer"
...
{`<CookieConsent
  title="Your privacy is important to us"
  description="We process your personal information..."
  privacyLink="/privacy"
  privacyLinkText="Privacy Policy"
  acceptLabel="Accept"
  optionsLabel="More Options"
